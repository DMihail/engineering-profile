          ...
          {EDUCATION.map((entry) => (
            <li key={entry.period || entry.institution}>
              <article className="panel px-5 py-4 sm:px-6 sm:py-5">
                <h3 className="text-base font-semibold text-foreground tracking-[-0.01em] text-balance">{entry.institution}</h3>
                <p className="text-sm text-text-secondary mt-1 text-pretty">{entry.field}</p>
                {entry.period && (
